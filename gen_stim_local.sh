...
# script for testing locally. save_image stores individual frames in separate directories.
vEnv=stim_gen
...
path="../runs/1/" #default=pwd path at which the stimuli should be stored
outer_path="../runs/" #outer path under which the pos and neg stim directories reside
# echo $path
# echo $outer_path
# last_sample=$(ls -lat | head -2 | tail -1 | awk '{print $9}' ) # | cut -d "_" -f 2)
